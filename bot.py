...
async def get_channel_id_from_input(bot, message):
    try:
        ...
        await message.reply_text("Kanal id ilə birlikdə cmd göndərin")
        return False
    if not str(a_id).startswith("-"):
        try:
            ...
            await message.reply_text("Yanlış kanal id")
            return False
    return a_id
...
async def main_handler(bot, message):
    ...
    try:
        ...
        return await message.reply_text("Məndən istifadə etmək üçün məni admin edin")
    if res:
        mention = f"@{message.sender_chat.username}" if message.sender_chat.username else message.chat_data.title
        await message.reply_text(text=f"{mention} banlandı.\n\n💡 O, yalnız öz profili ilə yaza bilər, başqa kanallarla yaza bilməz.",
...
async def start_handler(bot, message):
    await message.reply_text(text="""Hey! Sadəcə məni çata əlavə edin, mən də çata yazan kanalları bloklayacam,

daha çoxu üçün: /help .""",
                             reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Bots Channel", url=f"https://t.me/bybaghirov"),
                                                                 InlineKeyboardButton("Blog Channel", url=f"https://t.me/BaghirovBlog")]]),
                             disable_web_page_preview=True)

@JV_BOT.on_message(filters.command(["help"]) & filters.private)
async def help_handler(bot, message):
    await message.reply_text(text="""/ban [channel_id] : qrupa kanal olaraq mesaj göndərməyi qadağan edin.
/unban [channel_id] : qrupa kanal olaraq mesaj göndərməyi aktiv edin.
/add_whitelist [channel_id] : kanalı ağ siyahıya əlavə edin və avtomatik hərəkətlər üçün kanalı qoruyun.
/del_whitelist [channel_id] : kanalı ağ siyahıdan çıxarın.
/show_whitelist : Bütün ağ siyahıda olan kanalları göstərin.

daha çox kömək üçün: @TheBaghirov""",
                             reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Bots Channel", url=f"https://t.me/bybaghirov"),
                                                                 InlineKeyboardButton("Blog Channel", url=f"https://t.me/BaghirovBlog")]]),
                             disable_web_page_preview=True)
